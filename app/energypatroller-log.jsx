...
import { LIGHT_BULBS, CLASSROOMS, ROOMS_BY_FLOOR } from '../constants/appliances';
...
function makeRow() {
  ...
  rowId += 1;
  return { id: rowId, appliance: LIGHT_BULBS[0], onTime: now, offTime: later };
}

export default function EnergyPatrollerLog() {
  ...
  const [pickerTarget, setPickerTarget] = useState(null); // { id, field }
    ...
    Alert.alert(
      ...
      [
        { text: 'No', style: 'cancel', onPress: () => router.push('/') },
        {
          text: 'Yes',
          onPress: () =>
            router.push({ pathname: '/select-floor', params: { role: 'patroller' } }),
        },
      ]
    );
  }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 20 }}>
      ...
      <Text style={styles.pageTitle}>Energy Patrol - Light Log</Text>
        ...
        <View key={row.id} style={styles.entry}>
          <View style={styles.chipRow}>
            {LIGHT_BULBS.map((bulb) => (
              <Pressable
                key={bulb}
                style={[styles.chipSmall, row.appliance === bulb && styles.chipActive]}
                onPress={() => updateRow(row.id, 'appliance', bulb)}
              >
                <Text
                  style={[styles.chipText, row.appliance === bulb && styles.chipTextActive]}
                >
                  {bulb}
                </Text>
              </Pressable>
      ...
      <Pressable style={styles.addButton} onPress={addRow}>
        <Text style={styles.addButtonText}>+ Add another bulb</Text>
      </Pressable>
...
const styles = StyleSheet.create({
  ...
  dateText: { fontSize: 13, color: '#999', marginBottom: 4, fontWeight: '500' },
  pageTitle: { fontSize: 18, fontWeight: '700', color: '#1b1b1b', marginBottom: 16 },
  ...
  addButtonText: { color: '#4fc3f7', fontWeight: '600' },
  totalRow: {
    flexDirection: 'row',
  ...
  submitButtonText: { color: '#0d2b4e', fontSize: 16, fontWeight: '600' },
});